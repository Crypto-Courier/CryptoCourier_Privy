...
import { PointEntry, LeaderboardPointsData, MonthlyPointEntry } from '../types/leaderboard-controller-types';
...
export const createOrUpdateLeaderboardPoints = async (leaderboardPointsData: LeaderboardPointsData) => {
  ...
  const processedMonthlyPoints: MonthlyPointEntry[] = leaderboardPointsData.monthlyPoints;
      ...
      processedMonthlyPoints.forEach((monthlyPointEntry) => {
        ...
        if (existingMonthEntry) {
          const monthlyPointsMap = new Map<string, number>(
            existingMonthEntry.points.map((p: PointEntry) => [p.chainId, p.points])
          );

          monthlyPointEntry.points.forEach((newPointEntry) => {
            const existingPoints = monthlyPointsMap.get(newPointEntry.chainId) || 0;
            const updatedPoints = existingPoints + newPointEntry.points;
            monthlyPointsMap.set(newPointEntry.chainId, updatedPoints);
          });

          existingMonthEntry.points = Array.from(monthlyPointsMap).map(([chainId, points]) => ({
            chainId,
            points
          }));
        } else {
